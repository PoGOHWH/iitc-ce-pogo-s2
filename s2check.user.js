	...
	function wrapperPlugin(plugin_info) {
		...
		*/
		...
		function promptForPaste(callback) {
			...
			let width = Math.min(screen.availWidth, 360);
		...
		*/
		function displayCellSummary(cell) {
				...
				data.sort(sortByName).forEach(portal => {
					...
					if (scoreData != '') {
						wrapper.querySelector('.Pogo_Photos').addEventListener('input', function () {
							let update = portal.photos !== this.valueAsNumber && (portal.photos === 0 || this.valueAsNumber === 0);
			...
			let width = Math.min(screen.availWidth, 420);
			const container = dialog({
				id: 'PokemonList',
				html: div,
				width: width + 'px',
		...
		*/
		...
		thisPlugin.addStar = function (guid, lat, lng, name, type) {
			...
			if (type === 'pokestops') {
				...
				let hasPhoto = typeof pokestop.photos == 'undefined' || pokestop.photos > 0;
